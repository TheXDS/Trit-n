...
public class DataLayerMiddlewareTests
{
    ...
    public async Task Middleware_Test()
    {
        ...
        Assert.That(middleware.PrologueAction(CrudAction.Create, [new LoginCredential()])!.Reason, Is.EqualTo(FailureReason.Tamper));
        Assert.That(middleware.PrologueAction(CrudAction.Commit, [new LoginCredential()])!.Reason, Is.EqualTo(FailureReason.Tamper));

        prov.SecurityObject =  root;
        Assert.That(middleware.PrologueAction(CrudAction.Commit, null), Is.Null);
        Assert.That(middleware.PrologueAction(CrudAction.Create, [new LoginCredential()]), Is.Null);

        prov.SecurityObject = disabled;
        Assert.That(middleware.PrologueAction(CrudAction.Commit, null), Is.Null);
        Assert.That(middleware.PrologueAction(CrudAction.Create, [new LoginCredential()])!.Reason, Is.EqualTo(FailureReason.Forbidden));
    }
}
